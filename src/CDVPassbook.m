...
- (void)downloadPass:(NSURL*) url token:(NSString*)authToken success:(AddPassResultBlock)successBlock error:(void (^)(NSError *error))errorBlock;
...
- (BOOL)shouldOverrideLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType
{
    if(PKPassLibrary.isPassLibraryAvailable && [request.URL.pathExtension isEqualToString:@"pkpass"]) {
        [self downloadPass:request.URL token:nil success:nil error:nil];
        return YES;
    }
...
- (void)downloadPass:(CDVInvokedUrlCommand*)command
{
    ...
    NSString *tokenStr = [command argumentAtIndex:1];
    ...
    [self downloadPass:url token:tokenStr success:^(PKPass *pass, BOOL added){
...
- (void)downloadPass:(NSURL*) url token:(NSString*)authToken success:(AddPassResultBlock)successBlock error:(void (^)(NSError *error))errorBlock
{
    ...
    [request addValue:[@"Bearer " stringByAppendingString:authToken] forHTTPHeaderField:@"Authorization"];
